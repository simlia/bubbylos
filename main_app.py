...
class PulseScr(Screen):
    ...
    def next(self):
        if not self.next_screen: 
           ...
           global pl
           p1 = check_int(self.in result.text)
           if p1 == False or p1 <= 0:
              p1 = 0
...
class CheckSits(Screen):
     def init_(self, **kwargs): 
        ...
        self.btn = Button(text='Haчaть', size_hint=(0.3, 0.2), pos_hint=('center_x': 0.5)) 
    ...
    def sec_finished(self, *args):
        if self.lbl_sec.done:
           if self.stage == 0:
                ...
                self.btn.set_disabled(False) self.btn.text = 'Зaвеpшить' 
                self.next_screen = True
    def next(self):
         if not self.next_screen: 
                self.btn.set_disabled(True)
